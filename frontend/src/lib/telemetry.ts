...
/** Catmull-Rom interpolation — smooth positions/tangents through 4 Hz samples. */
function catmull(arr: number[], f: number): number {
  const n = arr.length;
  const i = Math.floor(f);
  if (i < 1 || i >= n - 2) return lerp(arr, f);
  const p0 = arr[i - 1];
  const p1 = arr[i];
  const p2 = arr[i + 1];
  const p3 = arr[i + 2];
...
export function sampleCar(race: RaceData, world: World, d: Driver, t: number): CarSample | null {
  ...
  const rx = catmull(tel.x, fc);
  const ry = catmull(tel.y, fc);
  const rz = catmull(tel.z, fc);
  const [wx, wy, wz] = toWorld(world, rx, ry, rz);

  // heading from a slightly forward sample on the same smooth curve
  const fAhead = Math.min(tel.n - 1, fc + 0.8);
  const ax = catmull(tel.x, fAhead);
  const ay = catmull(tel.y, fAhead);
  const dx = (ax - rx) / 10;
  const dz = -(ay - ry) / 10;
